...
Many fields of astronomy rely heavily on stellar ages; for example, the ages of planet hosts are necessary for planetary system evolution studies.
Unfortunately, observable stellar properties evolve slowly during the hydrogen burning phase and even high precision measurements often cannot pin down a star's age to within 20\% \citep{Soderblom2010}.
Of all age measurements, the most reliable and precise are for stars that lie in clusters where isochrones can be fitted to an ensemble of coeval stars with a range of masses.
...
Isochronally derived ages for field stars, where an isochrone is fitted to the spectroscopic properties of a single star, are much less precise than cluster ages and often have uncertainties of order 50\% or more.
A few alternatives to isochrone fitting exist.
Chromospheric emission and lithium abundance have been known to decay with stellar age since the 1960s.
\citet{Skumanich1972} first observed chromospheric emission, lithium depletion and rotational period to decay with the square-root of stellar age.
Later, a mass dependence was added to the period-activity-age relation.
...
Open clusters are therefore invaluable tools for calibrating empirical dating methods such as gyrochronology.
...
Cool Main Sequence (MS) stars are expected to lose angular momentum via a magnitised wind.
This wind is assumed to corotate with the stellar surface out to the Alfv\'{e}n radius \citep{Weber1967}, at which point point it decouples and angular momentum is lost from the star.
% In current angular momentum evolution models stellar winds are assumed to corotate with the stellar surface out to the Alfv\`{e}n radius \citep{Weber1967}, at which point the stellar wind decouples and angular momentum is lost from the star.
...
The timescale for convergence is around the age of the Hyades: 650 Myrs (\citet{Irwin2009}, \citet{Radick1987}) after this time rotation periods are \emph{independent} of their initial values.
...
The empirical gyrochronology relation of \citet{Barnes2007} can be written:
...
where $P$, $A$, $B$ and $V$ are rotation period (in days), age (in Myr),  and B and V band magnitudes respectively. The values of n, a, b and c are tabulated in \ref{tab:constants}.
This relation was further calibrated by \citet{Mamajek2008} and their revised parameters are also shown in table \ref{tab:constants}.
...
M dwarf angular momentum evolves over extremely long timescales and they often have not converged onto the mass-period-age plane, even after several Gyrs.
% not consistent with that of earlier type MS stars \citep{McQuillan2013}---this is presumed to be due to the fact that they are fully convective and have a different magnetic dynamo.
% Which is What?
...
It was calibrated using young open clusters aged 650 Myrs or less and the Sun; the clusters provided the period-mass dependence and the Sun provided the period-age dependence.
...
Note - apparently asteroseismic ages are easier to derive for older stars!
