...
% metallicity hyades = + 0.17
% praesepe = + 0.14
% Coma ber = - 0.05 collier cameron Cayrel de Strobel et al. (1997)
% NGC 6811 =
% \citep[text before][text after]{cite1, cite2}

% First, try to figure out where that extra multimodality is coming from
% second, try to incorporate new results into paper.
% should go back and see what happens if you add loggs in
% should go back and see what happens if you add praesepe back in
% Why are the asteroseismic stars still falling beneath the field stars?
...
% a 4.049308896064758301e-01 2.315343761444098591e-01 4.665800929069519043e-02
% n 5.532266497611999512e-01 1.965260267257695759e-02 7.078641653060913086e-02
% b 3.269872069358825684e-01 3.487592458724980693e-02 2.104172110557556152e-02
...
% Y 5.062700271606445312e+00 9.954514503479003906e-01 8.043809318542480824e-01
% V 2.103911161422729492e+00 1.046027421951293945e+00 5.943759679794311523e-01
% Z 1.610568237304687500e+01 7.457122802734375000e-01 7.779304885864242181e-01
% W 9.855091094970703125e+00 6.853249740600588780e-01 5.394491577148432526e-01
% X 1.421889591217041016e+01 4.147811775207522089e+00 3.596293449401855469e+00
% U 7.906614065170288086e+00 3.732418298721313477e+00 2.263251543045043945e+00
% Q 1.363826543092727661e-01 6.441044926643374469e-02 5.305185168981552124e-02
...
$a$, $b$ and $n$.
% and a best-fitting model which significantly overpredicted
% the age of the Sun.
...
% referee
We therefore exluded NGC 6811 from our sample and tried to fit a relation to
the remaining data, however, multi-modal posterior PDFs were still produced
until Praesepe was also removed from the sample.
...
\begin{figure}
\begin{center}
	\subfigure[Rotation period vs $B-V-c$.]{
            \label{fig:CF45}
	    \includegraphics[width=\columnwidth]{showCF45.pdf}
        }
	\subfigure[Rotation period vs age.]{
            \label{fig:HF45}
	    \includegraphics[width=\columnwidth]{showHF45.pdf}
        }
    \end{center}
...
\begin{figure*}
\begin{center}
\includegraphics[width=6in, clip=false, trim=0 0 0.5in 0]{small_triangleACHF.pdf}
...
\begin{center}
	\subfigure[0.5 Gyr (age of the Coma Ber)]{
            \label{fig:5}
	    \includegraphics[width=\columnwidth]{p_vs_bv0.pdf}
        }
	\subfigure[0.625 Gyr (age of the Hyades)]{
            \label{fig:625}
	    \includegraphics[width=\columnwidth]{p_vs_bv1.pdf}
        }
	\subfigure[2 Gyr]{
            \label{fig:2gyr}
	    \includegraphics[width=\columnwidth]{p_vs_bv2.pdf}
        }
	\subfigure[5 Gyr]{
            \label{fig:sungyr}
	    \includegraphics[width=\columnwidth]{p_vs_bv3.pdf}
        }
	\subfigure[8 Gyr]{
            \label{fig:8gyr}
	    \includegraphics[width=\columnwidth]{p_vs_bv4.pdf}
        }
    \end{center}
...
Parameters $a$ and $n$ are correlated and their posterior PDFs are bimodal.
The cause of this bimodality is clear when looking at the faint grey lines
representing draws from the parameter posterior PDFs shown in
figure \ref{fig:p_vs_a_solar}.
These lines represent draws from the posterior PDFs of $a$, $b$ and $n$.
The majority of these draws fall close to the best-fitting model which passes
neatly through the Sun, however a significant fraction fall below the line of
best-fit, as do Kepler asteroseismic stars.
The grey lines that fall beneath the best-fit model are draws from the smaller
peak in the posterior PDFs of $a$ and $n$ and they describe the relation
between rotation period and age for the Kepler asteroseismic stars.
% Although mostly consistent with this model they are not well
% described by it when taken in isolation.
It is clear that one set of gyrochronology parameters is not capable of
simulataneously describing the Sun and field stars and the Kepler asteroseismic
stars.
There is more than one possible explanation for this result.
Firstly, the asteroseismic ages could be systematically biased towards larger
ages, which could potentially result from the fact that the Kepler stars do
...
Thirdly, this could be a result of Malmquist bias (REFERENCE).
If there were a large spread in rotation periods for a given stellar mass and
age and, due to the bias that occurs because shorter periods are
easier to detect than longer periods, this broad range of rotation periods
might be truncated at some upper cut-off.
It would therefore appear as though Solar-colour stars were all rotating
rapidly when, in fact, there was a broad range of rotation periods but, due
to detection bias, only the rapid rotators appeared in our sample.
Finally, it is possible that the Kepler asteroseismic stars follow a
\emph{different} spin-down relation to the Sun and field stars. % not referee
This result is also reflected in \citet{Garcia2014} who model the AMP
Kepler asteroseismic stars from \citet{Metcalfe2014}.
They do not anchor their relation to the Sun and also find that their
best-fitting model falls below it.
...
The field stars in our sample are well represented by the best-fit
gyrochronology relation, which provides reassurance that the Sun \emph{is}
a typical star (at least amongst this set of nearby stars).
We leave the problem of how to appropriately treat the Sun as a single,
highly-precise data point in a sample also containing imprecise data
for future consideration.
Despite the fact that a lot of weight is attributed to the sun by virtue of its
...
reference age and are mostly dwarfs (stars with \logg$~<~$\subcut are not
shown).
Each of the faint grey lines represents a
sample drawn from the posterior probability distributions of $a$, $b$ and $n$.
...
% 18 Sco
% 3.66 0.64 22.7
% my age: [ 3.86228138  2.12584512  0.26586647]
% Barnes age: [ 3.52627769  0.55934568  0.47220735]
% M&H age: [ 3.70247763  0.74979761  0.60529662]
18 Sco      & $3.7 \pm 0.2$     & $3.9^{+2}_{-0.3}$ & $3.5^{+0.6}_{-0.5}$
	    & $3.7^{+0.8}_{-0.6}$ \\

% sun
% 4.568 0.65 26.9
% my age: [ 5.0383804   3.28831368  0.25822409]
% Barnes age: [ 4.6671184   0.71566045  0.60579257]
% M&H age: [ 4.80982801  0.95200645  0.76937419]
The Sun     & $4.568 \pm 0.001$ & $5.0^{+3}_{-0.3}$ & $4.7^{+0.7}_{-0.6}$
	    & $4.8^{+1}_{-0.8}$ \\

% alpha cen A
% 6.0 0.69 28.8
% my age: [ 5.35260438  2.70251808  0.95142837]
% Barnes age: [ 4.46912473  1.05603208  0.87463375]
% M&H age: [ 4.73895151  1.21466965  0.97232061]
Alpha Cen A & $6.0 \pm 1$       & $5^{+3}_{-1}$   & $4.5^{+1}_{-0.9}$
	    & $5\pm1$ \\

% alpha cen B
% 6.0 0.9 38.7
% my age: [ 6.38428453  3.13823942  1.58053316]
% Barnes age: [ 4.1987881   1.2790952   1.05350787]
% M&H age: [ 5.23974288  1.62087515  1.30076891]
Alpha Cen B & $6.0 \pm 1$       & $6^{+2}_{-2}$       & $4^{+1}_{-0.9}$
	    & $5^{+2}_{-1}$ \\

% 16 Cyg A
% 6.8 0.64 23.8
% my age: [ 4.32561587  2.11274775  0.64088381]
% Barnes age: [ 3.85547626  0.82050211  0.67894686]
% M&H age: [ 4.01776081  0.97509404  0.7763557 ]
16 Cyg A    & $6.8 \pm 0.4$     & $4.3^{+2}_{-0.6}$       & $3\pm2$
	    & $4\pm2$ \\

% 16 Cyg B
% 6.8 0.66 23.2
% my age: [ 4.06614579  3.16114975  2.06379298]
% Barnes age: [ 3.34291444  2.44475834  1.7423292 ]
% M&H age: [ 3.55423555  2.42069105  1.76114186]
