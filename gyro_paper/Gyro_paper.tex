...
\affil{$^2$School of Physics and Astronomy, Raymond and Beverly Sackler,
Faculty of Exact Sciences, Tel Aviv University, 69978, Tel Aviv, Israel}
\affil{$^3$Centre for Cosmology and Particle Physics, New York University, New
York, NY, USA}
\affil{$^4$School of Physics and Astronomy, University of Birmingham,
Edgbaston, Birmingham, B15 2TT, UK}
\affil{$^5$Stellar Astrophysics Centre (SAC), Department of Physics and
Astronomy, Aarhus University, DK-8000 Aarhus C, Denmark}
...
plane described by mass, rotation period and age, i.e. given any two of those
...
estimate the ages with a grid-based approach that utilized several different
...
mean large serparation, one can build up a density profile of the star and
provide a tighter constraint on its age.
Ages derived from individual oscillation mode measurements can have
uncertainties as small as 10\% \citep{Brown1994, SilvaAguirre2013}, however
measuring frequencies for individual oscillation modes is a manual process and
can only be applied in the highest signal-to-noise cases.
...
analysis since it concerns stars that are hotter than the Kraft break,
...
periods of the stars; flux variations were produced by active regions on the
...
\citet{Garcia2014}, and there is excellent agreement between rotation period
...
\begin{deluxetable}{lccc}
\tablewidth{0pc}
\tablecaption{Rotation periods and $B-V$ colours for field stars with
	precise ages.
References: (a) \citet{Metcalfe2012}, (b) \citet{Henry2000}, (c)
...
Notes: Rotation periods for 16 Cyg B and $\alpha$ Cen A and B were measured
...
18 Sco's age was measured using asteroseismology, however, although this
measurement was based chiefly on asteroseismic analysis, the rotation period
of the star was used as an additional constraint, so the age estimate is not
entirely independent of rotation period for this star.
...
from effective temperatures, photometric rotation periods, and
asteroseismically derived ages, and surface gravities.  Each measurement of
these properties is assumed to be independent with an associated Gaussian
uncertainty.
...
dwarfs: stars with effective temperatures above the Kraft-break, $T_{\rm{eff}}
...
a \logg$~$boundary of \subcut to mark the transition between dwarfs from
giants.
We trialed a range of boundary values, finding that \subcut minimised subgiant
...
Since the rotational properties of these stars are not interesting for the
purposes of gyrochronology calibration however, we also modelled them with
a normal distribution with mean and standard deviation, $Z$ and $U$, free
parameters.
We used a mixture model for the remaining population of stars: cool dwarfs and
misclassified, contaminating subgiants.
...
away---we could model everything at once.
...
boundaries would be free parameters of our model, however, since these two
populations were modelled with a relatively unconstraining normal
distributions, these boundary parameters would not be well behaved.
...
The left hand integral is the same for hot dwarfs, cool dwarfs and subgiants
alike.
...
\begin{deluxetable}{lccc}
\tablewidth{0pc}
\tablecaption{MAP values of a, b, c and n for individual clusters.
...
\tablewidth{0pc}
\tablecaption{MAP values of the parameters describing the populations of non-gyrochronological stars.
\label{tab:nuisance}}
\tablehead{
\colhead{Parameter}&
\colhead{MAP value}}
