...
There is an additional, contaminating, population of stars in our sample: rapid rotators.
A cluster of $\sim$ 20 stars have rotation periods of less than $\sim$ 5 days and do not lie on the standard gyrochronology mass-period-age plane.
These stars could be synchronised binaries.
Since our rotation period measurement method is biased towards fast rotators, we most probably measure rotation periods for all stars with periods $\lesssim$ 5 days.
% This could be quantised! - calculate your completeness as a function of period!!!
Therefore, we could consider this to be the almost complete population of rapid rotators in the entire asteroseismic sample.
...
The hot MS stars are defined as those with T$_{eff} > 6250$ and their rotation periods/ages are modelled as a log-normal distribution with mean, Y and variance, V inferred from the data.
Subgiants are defined as those stars with log $g < 4$ and their rotation periods/ages are modelled as a log-normal distribution with mean, Z and variance, U inferred from the data.
We use a mixture model for the remaining two populations of cool MS stars: those that follow the gyrochronology relations and those with unusually low rotation periods.
The fast rotators are treated as 'outliers' and modelled as another log normal distribution with mean, X and variance, W, again inferred from the data.
In our mixture model, an additional parameter, P, is the probability that each data point is an outlier.

Ideally both $T_K$ and the MS turnoff would be free parameters in our model.
However, since the hot MS and subgiant populations are being modelled as a much less constraining log-normal distribution, these boundary parameters would not be well behaved.
...
only including two regimes: the cool MS stars and the hot MS stars.
...
Sort out errorbars.
Get colour version working.
Get temp version working.

by the way - If you were forced to, couldn't you just write the paper now?
...
So should I use a flat prior centred around 0.54, or a Gaussian prior, or a combination of the two?
We want fairly unrestrictive priors on the remaining parameters.

\subsection{To do}

INCLUDE SUBS
The period errors really do seem to be too small, I have to make them bigger otherwise my MCMC doesn't converge.
Probably best to get more realistic uncertainties by taking something like the rms from quarter to quarter.
For now artificially inflating them is fine.

PRIORS
log(a) - should encompass both the barnes and mh values.
b. What is log(T) vs log(bv)?
log(teff) =  -0.327287044706 log(bv) +  3.68764284553
This might be the reason why I don't take 3 away from the ages to convert to Myrs...
It looks to me like the priors are being followed exactly!
This means the priors are very important and it means that we really should be adding in more data.
REALLY NEED A PLOT SHOWING THE MS COOL STARS!!!!
aarrgggh - this is because there ARE no stars in my sample at the moment!!!
%init2 and init_teff2 in include subs

I have already added Amy's stars into my sample.
What if you just cut out the ultra fast rotators and reinstate the cut?
remember that you will need to model another distribution of very rapid rotators
...
Given the fact that there is little differences between the posteriors of parameters n and b in the two cases, and both distributions look very similar to the prior distributions we take this to mean that there is not enough `power' in the data to constrain the values of n and b.
For this reason we need to add data from clusters and the Sun into our sample.
The cluster data should help constrain b - the index of the period - T$_{eff}$ relation and the Sun should help constrain n - the index of the period - age relation.

ADDING DATA
Need more data - get data from Barnes, M and H, James.
Cameron coma berenices
Meibom.
Raphael might share table.
Actually, can you just convert color to teff without logg and feh?
This would be SUPER useful!

GRID TK
Doing MCMC over teffs.
- Be VERY careful not to overwrite your triangle plots!
