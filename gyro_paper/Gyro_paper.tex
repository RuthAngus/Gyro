...
Of those 153, 13 of them have more precise asteroseismic ages from AMP modelling.
...
There are very few stars with temperatures below 6000 K (B-V $\approx$ 0.55) and of the low mass stars, most of them are old.
...
\begin{center}
    ...
    \end{center}
    \caption{ Photometric rotation period vs effective temperature and B-V colour for 153 Kepler asteroseismic targets.
     }
...
\caption{Colour, age and rotation periods of all 418 stars. Asteroseismic stars are black and cluster and field stars are red.}
\label{fig:3d}
\end{center}
\end{figure}
...
\caption{Photometric rotation period vs age for 153 Kepler targets (black) plus cluster and field stars (red). The blue stars have precise ages.}
\label{fig:p_vs_a}
\end{center}
\end{figure}
...
We postulate that there is a relationship between the (true) age, $A_n$, of a star and its (true) rotational period, $P_n$, and colour, $C_n$ (calculated from its true temperature), described by equation \ref{eq:plane}.
...
\begin{eqnarray}
	...
	+ \frac{P_b}{\sqrt{2\pi[U+\sigma_{Ai}^2]}} \exp \left( -\frac{[A_i - X]^2}{2[U+\sigma_{Ai}^2]}  \right)
\end{eqnarray}
...
\begin{eqnarray}
	\mathcal{L}_{hot} \propto \prod_{i=1}^N \left[ \frac{1}{\sqrt{2\pi[V+\sigma_{Ai}^2]}}
	\exp\left({-\frac{\left(\sum_{j=1}^J [A_{ij}- Y]\right)^2} {2[V+\sigma_{Ai}^2]}} \right)  \right] \\
	\nonumber \\
	\mathcal{L}_{subgiant} \propto \prod_{i=1}^N \left[ \frac{1}{\sqrt{2\pi[W+\sigma_{Ai}^2]}}
	\exp\left({-\frac{\left(\sum_{j=1}^J [A_{ij}- Z]\right)^2} {2[W+\sigma_{Ai}^2]}} \right)  \right] \nonumber \\
\end{eqnarray}

respectively, where Y and Z are the respective means and V and W the respective variances. The marginalised likelihood for a single star can be written as the sum of the individual star likelihoods over the three different regimes, k:
...
\caption{Marginalised posterior probability distributions of all parameters in our model. This figure was made using triangle.py \citep{Foreman-Mackey_triangle}. {\color{red}Note - this is just a placeholder and not the final plot!}}
...
{\color{red} to do: Something's not quite right here - these data need checking!}

\begin{figure}[ht]
\begin{center}
\includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/p_vs_bv_solar.png}
\caption{Rotation period vs B-V colour for cool dwarfs with age within 1$\sigma$ of the Sun's age, 4.568 Gyr. The grey stars are hotter than 6250 K and therefore did not contribute to the fitting process. The Sun is shown as a large grey circle.}
...
\caption{Rotation period vs age for cool dwarfs with colour within 1$\sigma$ of the Sun's; 0.65. The Sun is shown as a large grey circle.}
...
Precise modelling of asteroseismic stars using individual oscillation mode analysis is still in progress: precise ages will continue to be published over the coming months and years.
