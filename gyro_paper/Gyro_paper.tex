...
% \newcommand{\gyroa}{0.31}
% \newcommand{\aerrp}{0.04}
% \newcommand{\aerrm}{0.03}
% \newcommand{\gyron}{0.60}
% \newcommand{\nerrp}{0.01}
% \newcommand{\nerrm}{0.02}
% \newcommand{\gyrob}{0.40}
% \newcommand{\berrp}{0.01}
% \newcommand{\berrm}{0.05}
...
Best-fit values of $n$, $a$ and $b$ ($c$ was fixed at 0.4), reported in \citet{Barnes2007} are tabulated in \ref{tab:constants}.
Equation \ref{eq:Barnes2007-2} was further calibrated by \citet{Mamajek2008} using updated rotation period and age measurements of stars in open clusters $\alpha$ Per \citep{Prosser1995}, Pleiades \citep{Prosser1995, Krishnamurthi1998}, M34 \citep{Meibom2011_M34}, and Hyades \citep[Henry, private comm.,][]{Radick1987, Radick1995, Prosser1995, Paulson2004}.
...
discontinuity' in equation \ref{eq:Barnes2007-2}, at 0.4, \citet{Mamajek2008} allow it to be a free parameter in their model.
...
Notes: Rotation periods for 16 Cyg B and $\alpha$ Cen A and B were measured from variation in chromospheric emission lines.
High-resolution spectropolarimetric observations were used to measure a rotation period for 18 Sco.
The age of 16 Cyg B was measured with asteroseismology.
18 Sco's age was measured using asteroseismology, however, although this measurement was based chiefly on asteroseismic analysis, the rotation period of the star was used as an additional constraint, so the age estimate is not entirely independent of rotation period for this star.
An age for the $\alpha$ Cen system was estimated from spectroscopic observations with additional seismic constraints.
\label{tab:field}}
\tablehead{
\colhead{ID}&
\colhead{age}&
...
The \nastero$~$asteroseismic stars in our sample have $B-V$ colours converted from effective temperatures, photometric rotation periods, and asteroseismically derived ages, and surface gravities.
...
We trialed a range of boundary values, finding that \subcut minimised contamination whilst maximising the cool dwarf sample.
...
We modelled all three stellar populations simultaneously: cool dwarfs, hot dwarfs and subgiants.
...
equation \ref{eq:Barnes2007-2} (by `true' we mean the value an observable property would take, given extremely high signal-to-noise measurements).
...
\ref{eq:Barnes2007-2} and a likelihood calculated using a Gaussian mixture model likelihood function.
...
\begin{deluxetable}{lccc}
\tablewidth{0pc}
\tablecaption{Values of a, b, c and n for individual clusters.
...
\tablewidth{0pc}
\tablecaption{Values of the parameters describing the populations of non-gyrochronological stars.
\label{tab:nuisance}}
\tablehead{
\colhead{Parameter}&
\colhead{MAP value}}
...
Resulting MAP values of $a$, $b$ and $n$, with their 16th and 84th percentile uncertainties are tabulated in \ref{tab:constants} and the additional parameters of our model, describing the distributions of hot star and subgiant rotation periods, are presented in table \ref{tab:nuisance}.
Note the value of $Q$, the parameter describing the fraction of misclassified subgiants is $0.24^{+.06}_{-0.05}$: a quarter of the stars were misclassified as subgiants based on our \logg boundary.
...
inverted equation \ref{eq:Barnes2007-2}
