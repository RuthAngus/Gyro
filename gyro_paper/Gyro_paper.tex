...
We use rotation periods from \citet{Garcia2014} who used a combination of autocorrelation and wavelet transforms to measure surface rotation of the Kepler asteroseismic targets.
...
\citet{Garcia2014} used both Simple Aperture Photometry (SAP) data \citep{Thompson2013}, detrended using the Kepler Asteroseismic Data Analysis and Calibration Software (KADACS) \citep{Garcia2011} and Pre-Data Conditioning Maximum \emph{a Posteriori} (PDC-MAP) data in their analysis.
Moreover, to minimize the dependency of the correction soft- ware, we also use Kepler light curves processed using ¿Pre Data Conditioning Maximum a Posteriori¿ methods (PDC-MAP, e.g. Thompson et al. 2013).
Unfortunately, this methodology works in a quarter-by-quarter basis, which filters all periods close to 90 days, as well as, sometimes, everything longer than 20 days with a growing attenuation for periods longer than 3 days.
...
The Astrophysically Robust Correction (ARC) is another detrending method, developed by \citet{Roberts2013}, which preserves astrophysical signals.
However, it has not yet been applied to all Kepler quarters.

\citet{Garcia2014} use a combination of a wavelet transform method and an ACF method to measure the surface rotation periods of Kepler asteroseismic stars.
...
\citet{Garcia2014} only report rotation periods for stars with at least four full sinusoidal modulations in their light curves.
The GWPS and ACF methods are applied to both PDC-MAP and KADACS light curves.
They ensure that at least two of the four possible light curve and method combinations return rotation periods consistent to within 20\%.
A visual check is performed to confirm that the measured period is representative of the timescale of variability in the light curves and report final rotation periods measured from KADACS light curves using the GWPS method with HWHM uncertainties.
...
\begin{figure}[ht]
\begin{center}
	\subfigure[Fit to Coma Berenices and field stars]{
            \label{fig:CF45}
	    \includegraphics[width=2in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/showCF45.png}
        }
	\subfigure[Fit to Praesepe and field stars]{
            \label{fig:p_PF45}
	    \includegraphics[width=2in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/showp_PF45.png}
        }
	\subfigure[Fit to the Hyades cluster and field stars]{
            \label{fig:HF45}
	    \includegraphics[width=2in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/showHF45.png}
        }
	\subfigure[Fit to NGC 6811 and field stars. The period-colour relation of NGC 6811 is not well described by the gyrochronology relation.]{
            \label{fig:NF45}
	    \includegraphics[width=2in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/showNF45.png}
        }
    \end{center}
...
We fit a gyrochronology relation to each of the clusters individually together with the field stars.
...
The clusters clearly have different period-colour relations and the resulting parameters from the individual fits are not mutually consistent.
...
We therefore chose not to include it in the final fit.
...
We used the clusters and field stars as 4 separate training sets and the remaining data as the test set.
We also used just the asteroseismic targets as a test set
The resulting root-mean-squared (RMS) error for each trial is tabulated in \ref{tab:cross_val}.
Coma and the Hyades provide best training set for the remaining data - both when the other clusters are and are not included.
...
We also performed a stratified k-fold (SKF) cross validation with $K=5$.
In regular k-fold cross validation, the data is divided in to k subsets with each subset used as a test set, whilst the remaining data is used to train the model.
SKF cross validation is useful when the data is made up of different populations.
Instead of selecting test and train data from the entire data set at random, a consistent percentage of stars is drawn from each population.
This reduces the variance of the cross validation score.
The results of the 5-fold SKF are tabulated in \ref{tab:skf}.

\begin{deluxetable}{lcc}
\label{tab:cluster_results}
\tablewidth{0pc}
\tablecaption{Results of individual cluster cross validation. C = Coma Berenices, P = Praesepe, H = Hyades, N = NGC 6811, F = field stars and A = Kepler asteroseismic targets.}
\tablehead{
\colhead{Training data}&
\colhead{Test data}&
\colhead{RMS}}
...
Why do NGC 6811 and Praesepe have such different period-colour relations?
Pulsators in NGC 6811?
