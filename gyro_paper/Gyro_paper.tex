...
extrememly useful.
Gyrochronology is a dating method that utilises the (theoretically)
...
16 Cyg B & 6.4 $\pm$ 0.4 & 23.2 $^{+11.5}_{-3.2}$ & 0.66 $\pm$ 0.01 &
\citet{Metcalfe2012}, \citet{Davies2014}, \citet{Moffett1979} \\
...
describe the observed properties of our sample stars is due to the simplifying
assumptions that go into this relationship.
...
a star since it can impact its internal structure.
We have not attempted to calibrate a metallicity-dependent gyrochronology
relation, partly there are no precise metallicity measurements for the
majority of the Kepler stars, on which this study is chiefly based.
We anticipate that the majority of stars for which this new gyrochronology
relation will be most useful are Kepler stars, or stars targeted by missions
like Kepler, again, the majority of which are unlikely to have precise
metallicities.  % referee
...
posterior PDFs are multimodal. The main peaks in the posterior PDFs of $a$ and
$n$ correspond to a fit to the Sun and field stars. The smaller peak
corresponds to the a fit to the Kepler asteroseismic stars.
...
% multi-modal.
Parameters $a$ and $n$ are correlated and their posterior PDFs are multimodal.
The cause of this multimodality is clear when the values of the parameters
at the two peaks are plugged into the gyrochronology relation and plotted
alongside the data.
When the median values of the parameters are input into the model, the
resulting line shown in figure \ref{fig:p_vs_a_solar} is a good fit to the Sun
and field stars.
When parameter values corresponding to the smaller peak in the posterior PDF
are input into the model, the resulting line appears to be a good fit to the
Kepler asteroseismic stars.  % not referee
The Kepler asteroseismic stars in figure \ref{fig:p_vs_a_solar} fall below the
line described by the gyrochronology relation with median parameter values.
Although mostly consistent with the best-fitting model, they are not well
