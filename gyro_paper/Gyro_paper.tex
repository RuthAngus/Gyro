...
This paper is laid out as follows: the data and our photometric rotation period measurement method are described in \textsection~\ref{sec:asteroseismic_targets}, the calibration and model fitting process is outlined in \textsection~\ref{sec:gyro_cal} and the results are discussed in \textsection~\ref{sec:results}.
...
These oscillations can be detected in short-cadence Kepler data, and a fourier transform of the time-series shows a series of peaks, corresponding to discrete oscillation mode.
...
The ages of 505 stars were calculated from measurements of the mean large frequency separation, $\Delta\nu$ and the maximum of the gaussian envelope, $\nu_{max}$ and published in \citet{Chaplin2013}
They are published with two sets of effective temperatures: one is derived using an Infra-Red Flux Method (IRFM) calibration (\citet{Casagrande2010}, \citet{SilvaAguirre2012}) and the other from a recalibration of the SDSS griz filter KIC photometry by \citet{Pinsonneault2012} using Yale Rotating Stellar Evolution Code (YREC) models \citep{Demarque2008}.
We use the second set of temperatures since they temperatures are slightly more precise.
% test: is our analysis sensitive to this choice?
The asteroseismic ages in \citet{Chaplin2013} have typical uncertainties of $\approx$ 35\%; however, for some of these stars it will be possible to derive more precise ages.
If the frequency of each oscillation mode is measured individually, one can build up a density profile of the star and provide a tighter constraint on its age.
Ages derived from individual oscillation mode measurements can have uncertainties as small as 10\%, however the process of measuring frequencies for individual oscillation modes is a manual, more time consuming process and can only be applied in the highest signal-to-noise cases.
...
The AMP used the `peak bagging' technique \citep{Appourchaux2012} to measure individual oscillation mode frequencies
...
\subsection{Rotation Period Measurements}
...
sinusoidal models \citep{Zechmeister}) autocorrelation is much better suited to signals that are not sinusoidal or strictly periodic and is more effective at distinguishing a true signal from its harmonics and sub-harmonics.
...
The first ACF peak is small when superimposed onto this trend, the period will be underestimated as the central peak value will be shifted towards smaller periods.
...
Of the 505 targets in the original sample 145 rotation periods were reliably measured using the above process.
We added an additional 31 targets with rotation periods measured by \citet{McQuillan_2014}, increasing our sample to 176.
Of those 176, 13 of them have more precise asteroseismic ages from AMP modelling.

% \begin{figure}[ht]
% \begin{center}
% \includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/logg_vs_t_paper.png}
% \caption{\logg vs \teff for the 176 asteroseismic stars. Stars with \teff $>$ 6250 K are red and those with \logg $<$ 4.0 are blue. {\color{red}To do: add ASTEC isochrones and extra data}}
% \label{fig:p_vs_a}
% \end{center}
% \end{figure}
...
% \begin{figure}[ht]
% \begin{center}
% \includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/p_vs_a_paper.png}
% \caption{Photometric rotation period vs asteroseismic age for 176 Kepler targets. {\color{red}to do: add more precise data in a different colour}}
% \label{fig:p_vs_a}
% \end{center}
% \end{figure}
...
% \begin{figure}[ht]
% \begin{center}
% \includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/p_vs_t_paper.png}
...
% \begin{figure}[ht]
% \begin{center}
% \includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/users/angusr/python/gyro/plots/p_vs_bv_paper.png}
% \caption{Photometric rotation period vs B-V colour for 176 Kepler targets.}
% \label{fig:p_vs_a}
% \end{center}
% \end{figure}
...
A further 5 stars with precise age measurements were added to the sample: 16 Cyg B, Alpha Cen A and B, 18 Sco and, of course, the Sun.
...
Despite having effective temperatures for the asteroseismic targets, we converted $T_{eff}$ to B-V colours, using the relation in \citet{Sekiguchi2000}.
This conversion is imprecise since the metallicites of the asteroseismic targets are the field average and not calculated per star, however we decided that converting $T_{eff}$ to colour would be more precise than converting cluster star colours to $T_{eff}$, since we have more information about the asteroseismic sample.

The entire set of 441 stars is shown in figure \ref{fig:3d}. Asteroseismic targets are shown in black, with supplementary cluster and field stars in red.

% \begin{figure}[ht]
% \begin{center}
% \includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/Python/Gyro/plots/3d_angled.png}
% \caption{Colour, age and rotation periods of all 441 asteroseismic (black), cluster and field stars (red).}
% \label{fig:3d}
% \end{center}
% \end{figure}
...
% \caption{Photometric rotation period vs B-V colour for 176 Kepler targets (black) plus cluster and field stars (red).}
% \label{fig:3d}
% \end{center}
% \end{figure}
...
% \caption{Photometric rotation period vs age for 176 Kepler targets (black) plus cluster and field stars (red).}
% \label{fig:3d}
% \end{center}
% \end{figure}
...
where n, a, b and c are constants, taking values tabulated in \ref{tab:constants}.
...
\begin{eqnarray}
	\mathcal{L} \propto \prod_{i=1}^N \left[ \frac{1-P_b}{\sqrt{2\pi\sigma_{Ai}^2}}
	\exp\left({-\frac{\left(\sum_{j=1}^J [A_{ij}- (P_{ij} \times \frac{1}{a}([B-V]_{ij}-c)^{-b})^{\frac{1}{n}}]\right)^2} {2\sigma_{Ai}^2}} \right)  \right] \nonumber \\
	+ \frac{P_b}{\sqrt{2\pi[Z+\sigma_{Ai}^2]}} \exp \left( -\frac{[A_i - U]^2}{2[Z+\sigma_{Ai}^2]}  \right)
\end{eqnarray}

where $P_b$ is the probability that a star is drawn from the background log-normal distribution, with mean, U and variance, Z.

The likelihood function for the hot and subgiant regimes can be written

\begin{eqnarray}
	\mathcal{L} \propto \prod_{i=1}^N \left[ \frac{1}{\sqrt{2\pi[V+\sigma_{Ai}^2]}}
	\exp\left({-\frac{\left(\sum_{j=1}^J [A_{ij}- Y]\right)^2} {2[V+\sigma_{Ai}^2}]} \right)  \right] \nonumber \\
\end{eqnarray}

respectively, where Y and Z are the respective means and V and W the respective variances.

We used emcee \citep{Foreman-Mackey2013} to explore the posterior probability distributions of the model parameters, $\theta$.
...
Alternatively, the ACF method could be measuring rotation periods that are too small.
% could test this
...
It is, possible that systematic overestimation of the asteroseismic star's ages, or underestimation of their rotation periods is responsible for this discrepancy.

Precise modelling of asteroseismic stars using individual oscillation mode analysis is still in progress: more precise ages will continue to be published over the coming months and years.

The code used in this project can be found at https://github.com/dfm/noisy-plane.
We would like to thank Eric Mamajek, Marc Pinsonneault, Sydney Barnes, Steve Kawaler, Jerome Bouvier and Daniel Mortlock and David Hogg for their useful insight and discussion.
...
% \begin{center}
% \includegraphics[width=6in, clip=true, trim=0 0 0.5in 0]{/Users/angusr/angusr/ACF/ACF2/PDCQ3_output/plots_acf/3223000_full.png}
% \caption{PDC-MAP and 'processed' light curves for KIC 3223000 with its ACF.}
% \label{fig:lc}
% \end{center}
% \end{figure}
